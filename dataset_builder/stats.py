...
) -> tuple[int, int, Counter[int], list[str]]:
    if not path.exists():
...
def collect_dataset_stats(dataset: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    for record in records:
        ...
        try:
            ...
            count, invalid, valid_classes, label_issues = _read_label(
                label, len(config["classes"])
            )
            if not label.exists():
                missing_labels += 1
            if (
                label.exists()
                and not label.read_text(encoding="utf-8", errors="replace").strip()
            ):
                empty_labels += 1
            objects += count
            invalid_labels += invalid
            max_objects = max(max_objects, count)
            class_counts.update(valid_classes)
            for issue in label_issues:
                issues.append({"path": str(image), "issue": issue})
        except OSError as exc:  # Defensive: collect filesystem integrity issues.
            issues.append({"path": str(image), "issue": str(exc)})
    expected_labels = {
        image.relative_to(image_root).with_suffix(".txt")
        for image in (record["path"] for record in records)
    }
    actual_labels = (
        {
            label.relative_to(dataset / "labels")
            for label in (dataset / "labels").rglob("*.txt")
        }
    ...
    for label in sorted(orphan_labels):
        issues.append(
            {
                "path": str(dataset / "labels" / label),
                "issue": "label has no matching image",
            }
        )
    ...
    return {
        ...
        "corrupt_images": len(records) - len(valid),
        "dataset_bytes": sum(record["file_size"] for record in records),
        "average_resolution": {
            "width": round(sum(r["width"] for r in valid) / len(valid), 1)
            if valid
            else 0,
            "height": round(sum(r["height"] for r in valid) / len(valid), 1)
            if valid
            else 0,
        },
        "resolutions": dict(resolutions),
        "formats": dict(formats),
        "images_labeled": len(records) - missing_labels,
        "images_missing_labels": missing_labels,
        "empty_labels": empty_labels,
        "total_objects": objects,
        "objects_per_class": {
            config["classes"][key]: value
            for key, value in class_counts.items()
            if 0 <= key < len(config["classes"])
        },
        "average_objects_per_image": round(objects / len(records), 2) if records else 0,
        "max_objects_per_image": max_objects,
        "invalid_labels": invalid_labels,
        "issues": issues,
        "labels_missing_images": len(orphan_labels),
